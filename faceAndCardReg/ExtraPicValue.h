...
#ifndef FACEREG_EXTRAPICVALUE_H
#define FACEREG_EXTRAPICVALUE_H
...
public:
	ExtraPicValue();
	ExtraPicValue(const cv::Mat &src);
	QList<double> doFaceCompare(const cv::Mat &source,const cv::Mat &compare);
	void extra128DMatrix(const cv::Mat &source);
	~ExtraPicValue();
private:
	anet_type  net;
	cv::Mat  source;
